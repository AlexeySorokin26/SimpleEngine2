...
namespace SimpleEngine {
		...
		, m_height(h)
	{
		...
		// TODO make it work learn it
		////we create text object
		//// (target could be cube, how many handles, handle)
		//glCreateTextures(GL_TEXTURE_2D, 1, &m_id);
		//// allocate memory on gpu
		//// (handle, how many mipmaps, internval fomat, size)
		//glTextureStorage2D(m_id, mip_levels, GL_RGBA, m_width, m_height);
		//// load texture into gpu from cpu
		//// (handle, level mip map, offset in memory in x and y to fill whatever part of image, w, h, data format, data type, pointer to data)
